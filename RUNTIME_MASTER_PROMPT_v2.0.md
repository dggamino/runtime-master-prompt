...
| `PROVISIONAL` | Hecho introducido en sesión, pendiente de consolidación. Persiste entre sesiones hasta veredicto final del Custodio. | Sí, hasta veredicto final | Curador (sin aprobación del Custodio) |
...
MAJOR:  Reescribe HARD_CANON existente o redefine regla inviolable → solo el Custodio
MINOR:  Bloque completo incorporado / conjunto de HARD_CANON consolidado → Curador + Custodio
PATCH:  PROVISIONAL → HARD_CANON (nuevo) / corrección SOFT_CANON → Curador veredicto + aprobación Custodio

Nota: PATCH permite promover PROVISIONAL a HARD_CANON nuevo con aprobación del Custodio.
      MAJOR se reserva exclusivamente para modificar o reescribir HARD_CANON ya existente.
```
...
Regla R3: aprobado_por es null para tipo PROVISIONAL — no requiere aprobación del Custodio.
Regla R4: todos los valores de tipo canónico van entre comillas en JSON (estandarización).
```
...
*RUNTIME MASTER PROMPT v2.0*
*Resoluciones R1-R4 aplicadas — aprobadas por Custodio — 2026-06-01*
